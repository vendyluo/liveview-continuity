...
The package requires Elixir 1.18 or newer and Phoenix LiveView 1.1 or newer. Add the LiveView compiler to the consuming project:
...
Client-owned reflections use stable `data-lvc-*` styling hooks. Only `data-lvc-open`, `data-lvc-dismiss-reason`, `data-lvc-active`, and item `tabindex` are protected with LiveView's narrow `JS.ignore_attributes/1` mechanism. Descendants remain server-patchable.
...
Also run `mix format --check-formatted`, `mix compile --warnings-as-errors`, `mix test`, `node --check playwright/conformance.mjs`, `mix hex.build`, and `mix docs`. The fixture uses loopback port 4140 and the shipped component plus its compiler-extracted colocated hook.
